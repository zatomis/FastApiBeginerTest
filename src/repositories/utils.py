...
def rooms_ids_for_booking(date_from,
                          date_to,
...
# left join rooms_count on rooms.id = rooms_count.room_id)
    ...
    select(
        ...
        (RoomsORM.quantity - func.coalesce(query_rooms_count.c.rooms_booked, 0)).label("rooms_left"),
    )
    .select_from(RoomsORM)
    .outerjoin(query_rooms_count, RoomsORM.id == query_rooms_count.c.room_id)
...
# select * from rooms_left_not_null_table where rooms_left > 0
    ...
    rooms_ids_in_hotel = (
        rooms_ids_in_hotel
        .subquery(name="rooms_ids_in_hotel")
    )
...
# получим только те номера, которые не забронированы и относятся к конкретному отелю
    ...
    .filter(rooms_left_not_null_table.c.rooms_left > 0,
