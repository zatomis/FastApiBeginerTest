...
class RoomsFacilitiesRepository(BaseRepository):
    ...
    async def set_room_faclities1(self, room_id: int, faclities_ids: list[int]):
        ...
        get_facilities_ids_query = select(self.model.facility_id).filter_by(
            room_id=room_id
        )
        current_res = await self.session.execute(get_facilities_ids_query)
        current_faclities_ids: list[int] = current_res.scalars().all()
        ids_to_delete: list[int] = list(
            set(current_faclities_ids) - set(faclities_ids)
        )
        ids_to_add: list[int] = list(
            set(faclities_ids) - set(current_faclities_ids)
        )
        ...
        if ids_to_add:
            insert_m2m_facilities_stmt = insert(
                self.model
            ).values(  # вставляем массив словарей
                [
                    {"room_id": room_id, "facility_id": f_id}
